...
class Actor(nn.Module):
    ...
    def forward(self, batch_states, feasible_actions):
        ...
        self.n_nodes_per_state=n_nodes_per_state
        ...
        return sampled_actions, log_prob,pi

    def evaluate(self,batch_states, feasible_actions, sampled_actions):
        device = "cuda" if torch.cuda.is_available() else "cpu"

        _,_,action_probs = self.forward(batch_states,feasible_actions)
        dist = Categorical(action_probs)

        actions = torch.tensor([action[0] * self.n_nodes_per_state + action[1] for action in sampled_actions])
        actions = actions.to(device=device, dtype=torch.long).unsqueeze(-1)
...
class Critic(nn.Module):
    def __init__(self, in_dim, hidden_dim, embedding_l, embedding_type='gin', heads=4, dropout=0.6):
        super(Critic, self).__init__()
        self.embedding_l = embedding_l
        self.embedding_type = embedding_type

        # Depending on the embedding type, we use the appropriate embedding module
        if self.embedding_type == 'gin':
            self.embedding = GIN(in_dim=in_dim, hidden_dim=hidden_dim, layer_gin=embedding_l)
        elif self.embedding_type == 'dghan':
            self.embedding = DGHAN(in_dim=in_dim, hidden_dim=hidden_dim, dropout=dropout, layer_dghan=embedding_l,
                                   heads=heads)
        elif self.embedding_type == 'gin+dghan':
            self.embedding_gin = GIN(in_dim=in_dim, hidden_dim=hidden_dim, layer_gin=embedding_l)
            self.embedding_dghan = DGHAN(in_dim=in_dim, hidden_dim=hidden_dim, dropout=dropout, layer_dghan=embedding_l,
                                         heads=heads)
        else:
            raise Exception('embedding type should be either "gin", "dghan", or "gin+dghan".')
        ...
        if self.embedding_type == 'gin+dghan':
            self.value_head = nn.Linear(hidden_dim*2, 1)  # hidden_dim * 4 because of concatenated embeddings
        else:
            self.value_head = nn.Linear(hidden_dim*2, 1)  # hidden_dim * 2 for a single type of embedding

    def forward(self, batch_states, feasible_actions):
        if self.embedding_type == 'gin':
            _, graph_embed = self.embedding(batch_states.x, add_self_loops(
                torch.cat([batch_states.edge_index_pc, batch_states.edge_index_mc], dim=-1))[0], batch_states.batch)
        elif self.embedding_type == 'dghan':
            _, graph_embed = self.embedding(batch_states.x, add_self_loops(batch_states.edge_index_pc)[0],
                                            add_self_loops(batch_states.edge_index_mc)[0], len(batch_states.x))
        elif self.embedding_type == 'gin+dghan':
            _, graph_embed_gin = self.embedding_gin(batch_states.x,
                                                                 add_self_loops(torch.cat([batch_states.edge_index_pc,
                                                                                           batch_states.edge_index_mc],
                                                                                          dim=-1))[0],
                                                                 batch_states.batch)
            _, graph_embed_dghan = self.embedding_dghan(batch_states.x,
                                                                       add_self_loops(batch_states.edge_index_pc)[0],
                                                                       add_self_loops(batch_states.edge_index_mc)[0],
                                                                       len(feasible_actions))
            graph_embed = torch.cat([graph_embed_gin, graph_embed_dghan], dim=-1)
            batch_size = graph_embed.shape[0]
            graph_embed=graph_embed.reshape(batch_size, -1)

        # Calculate value of the graph embedding
